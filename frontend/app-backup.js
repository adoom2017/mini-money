...
const AuthPage = ({ lang, t, login }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                login(data);
                setToastMessage(t(isLogin ? 'login_success' : 'register_success'));
...
const App = () => {
    ...
    const fetchCategories = async () => {
        try {
            const response = await fetch('/api/categories'); // Categories are public, no auth needed
            if (response.ok) {
                const data = await response.json();
    ...
    React.useEffect(() => {
        // Always fetch categories since they're public
        fetchCategories();
    }, []);
    ...
    const commonProps = { lang, t, allCategories, categoryIconMap, fetchWithAuth };
    ...
    if (!isAuthenticated) {
        return <AuthPage lang={lang} t={t} login={login} />;
    }
    ...
    return (
        <div className="container mt-4">
            <div className="d-flex justify-content-between align-items-center mb-2">
                ...
                <div>
                    <button className="btn btn-outline-secondary btn-sm me-2" onClick={() => setLang(lang === 'en' ? 'zh' : 'en')}>
                        {lang === 'en' ? '中文' : 'English'}
                    </button>
