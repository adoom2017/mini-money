...
const AssetsPage = ({ lang, t, fetchWithAuth, showToast }) => {
    ...
    const handleAddRecord = async () => {
        ...
        try {
            ...
            const existingRecord = selectedAsset.records.find(record => record.date === newRecord.date);
            
            let response;
            if (existingRecord) {
                // Update existing record
                response = await fetchWithAuth(`/api/assets/${selectedAsset.id}/records/${existingRecord.id}`, {
                    method: 'PUT',
                    body: JSON.stringify({
                        date: newRecord.date,
                        amount: amount
                ...
                // Create new record
                response = await fetchWithAuth(`/api/assets/${selectedAsset.id}/records`, {
                    method: 'POST',
                    body: JSON.stringify({
                        date: newRecord.date,
                        amount: amount
            ...
            if (response.ok) {
                ...
                const updatedAssets = assets.map(asset => {
                    if (asset.id === selectedAsset.id) {
                        ...
                        if (existingRecord) {
                            ...
                            newRecords = asset.records.map(r => r.id === existingRecord.id ? record : r);
                        } else {
                            // Add new record
                            newRecords = [...asset.records, record];
                        }
                ...
                showToast(existingRecord ? '记录更新成功' : '记录添加成功', 'success');
            } else {
                const errorData = await response.json();
                showToast(errorData.error || (existingRecord ? '更新记录失败' : '添加记录失败'), 'error');
            }
        } catch (error) {
