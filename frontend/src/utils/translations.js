const translations = {
    en: {
        // General
        currencySymbol: "$",
        title: "Simple Bookkeeping",
        bookkeeping: "Bookkeeping",
        statistics: "Statistics",
        assets: "Assets",
        totalIncome: "Total Income",
        totalExpense: "Total Expense",
        balance: "Balance",
        expense: "Expense",
        income: "Income",
        amount: "Amount",
        category: "Category",
        description: "Description (Optional)",
        description_placeholder: "Add a note...",
        save: "Save",
        recentTransactions: "Recent Transactions",
        no_transactions_yet: "No transactions yet",
        // Auth
        login: "Login",
        register: "Register",
        logout: "Logout",
        username: "Username",
        email: "Email",
        password: "Password",
        username_placeholder: "Enter username",
        email_placeholder: "Enter email",
        password_placeholder: "Enter password",
        login_title: "Login to Your Account",
        login_subtitle: "Welcome back! Please sign in to continue",
        register_title: "Create New Account",
        register_subtitle: "Join us today and start managing your finances",
        have_account: "Already have an account?",
        no_account: "Don't have an account?",
        login_success: "Login successful!",
        register_success: "Registration successful!",
        user_profile: "User Profile",
        welcome: "Welcome",
        change_avatar: "Change Avatar",
        upload_avatar: "Upload Avatar",
        avatar_updated: "Avatar updated successfully!",
        user_settings: "User Settings",
        profile: "Profile",
        change_password: "Change Password",
        change_email: "Change Email",
        current_password: "Current Password",
        new_password: "New Password",
        confirm_password: "Confirm Password",
        new_email: "New Email",
        enter_password_to_confirm: "Enter password to confirm",
        update_password: "Update Password",
        update_email: "Update Email",
        updating: "Updating...",
        password_updated: "Password updated successfully!",
        email_updated: "Email updated successfully!",
        logged_out: "Logged out successfully!",
        settings: "Settings",
        user_config: "User Configuration",
        not_set: "Not set",
        join_date: "Join Date",
        unknown: "Unknown",
        click_avatar_to_change: "Click avatar tab to change avatar",
        username_cannot_change: "Username cannot be changed",
        click_email_tab_to_change: "Click email tab to change email",
        upload_new_avatar: "Upload New Avatar",
        avatar_requirements: "Support JPG, PNG formats, file size less than 2MB",
        remove_avatar: "Remove Avatar",
        confirm_new_password: "Confirm New Password",
        password_min_6_chars: "Password must be at least 6 characters",
        confirm_logout: "Are you sure you want to log out?",
        passwords_not_match: "Passwords do not match",
        password_min_length: "Password must be at least 6 characters long",
        fill_all_fields: "Please fill in all fields",
        file_too_large: "File size must be less than 2MB",
        close: "Close",
        // Statistics Page
        statistics_title: "Monthly Statistics",
        select_year: "Select Year",
        select_month: "Select Month",
        monthly_summary: "Monthly Summary",
        category_breakdown: "Category Breakdown",
        loading: "Loading...",
        initializing_app: "Initializing application...",
        error_fetching_stats: "Error fetching statistics.",
        no_data_period: "No data for this period.",
        // Records Page
        records: "Records",
        records_title: "Transaction Records",
        all_records: "All Records",
        filter_by_type: "Filter by Type",
        all_types: "All Types",
        search_description: "Search by description...",
        no_records: "No records found.",
        edit: "Edit",
        delete: "Delete",
        confirm_delete: "Are you sure you want to delete this transaction?",
        // Categories
        food: "Food", medical: "Medical", transport: "Transport", housing: "Housing", snacks: "Snacks",
        learning: "Learning", communication: "Communication", social: "Social", investment: "Investment", shopping: "Shopping",
        salary: "Salary", part_time: "Part-time", financial: "Financial", red_packet: "Red Packet", other: "Other",
    },
    zh: {
        // General
        currencySymbol: "¥",
        title: "极简记账",
        bookkeeping: "记账",
        statistics: "统计",
        assets: "资产",
        totalIncome: "总收入",
        totalExpense: "总支出",
        balance: "余额",
        expense: "支出",
        income: "收入",
        amount: "金额",
        category: "分类",
        description: "备注 (可选)",
        description_placeholder: "添加备注...",
        save: "保存",
        recentTransactions: "最近的交易",
        no_transactions_yet: "暂无交易记录",
        // Auth
        login: "登录",
        register: "注册",
        logout: "退出登录",
        username: "用户名",
        email: "邮箱",
        password: "密码",
        username_placeholder: "请输入用户名",
        email_placeholder: "请输入邮箱",
        password_placeholder: "请输入密码",
        login_title: "登录您的账户",
        login_subtitle: "欢迎回来！请登录以继续使用",
        register_title: "创建新账户",
        register_subtitle: "加入我们，开始管理您的财务",
        have_account: "已有账户？",
        no_account: "还没有账户？",
        login_success: "登录成功！",
        register_success: "注册成功！",
        user_profile: "用户资料",
        welcome: "欢迎",
        change_avatar: "更换头像",
        upload_avatar: "上传头像",
        avatar_updated: "头像更新成功！",
        user_settings: "用户设置",
        profile: "个人资料",
        change_password: "修改密码",
        change_email: "修改邮箱",
        current_password: "当前密码",
        new_password: "新密码",
        confirm_password: "确认密码",
        new_email: "新邮箱",
        enter_password_to_confirm: "输入密码确认",
        update_password: "更新密码",
        update_email: "更新邮箱",
        updating: "更新中...",
        password_updated: "密码更新成功！",
        email_updated: "邮箱更新成功！",
        logged_out: "已成功退出！",
        settings: "设置",
        user_config: "用户配置",
        not_set: "未设置",
        join_date: "加入时间",
        unknown: "未知",
        click_avatar_to_change: "点击头像标签页更换头像",
        username_cannot_change: "用户名不可修改",
        click_email_tab_to_change: "点击邮箱标签页修改邮箱",
        upload_new_avatar: "上传新头像",
        avatar_requirements: "支持 JPG、PNG 格式，文件大小不超过 2MB",
        remove_avatar: "移除头像",
        confirm_new_password: "确认新密码",
        password_min_6_chars: "密码至少 6 个字符",
        confirm_logout: "确定要登出吗？",
        passwords_not_match: "两次输入的密码不一致",
        password_min_length: "密码至少需要 6 个字符",
        fill_all_fields: "请填写所有字段",
        file_too_large: "文件大小不能超过 2MB",
        close: "关闭",
        // Statistics Page
        statistics_title: "月度统计",
        select_year: "选择年份",
        select_month: "选择月份",
        monthly_summary: "本月小结",
        category_breakdown: "分类排行",
        loading: "加载中...",
        initializing_app: "正在初始化应用...",
        error_fetching_stats: "加载统计数据失败。",
        no_data_period: "该时段无数据。",
        // Records Page
        records: "记录",
        records_title: "交易记录",
        all_records: "全部记录",
        filter_by_type: "按类型筛选",
        all_types: "全部类型",
        search_description: "搜索备注...",
        no_records: "暂无记录。",
        edit: "编辑",
        delete: "删除",
        confirm_delete: "确定要删除这条记录吗？",
        // Categories
        food: "早午晚餐", medical: "医疗", transport: "出行交通", housing: "住房", snacks: "零食烟酒",
        learning: "学习", communication: "通讯", social: "社交", investment: "金融投资", shopping: "购物",
        salary: "工资", part_time: "兼职", financial: "理财", red_packet: "红包", other: "其他",
    }
};

// Translation helper function
const createTranslator = (lang) => (key) => translations[lang][key] || key;

// Export for global access
window.translations = translations;
window.createTranslator = createTranslator;
